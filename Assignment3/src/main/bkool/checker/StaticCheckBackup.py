...
class StaticChecker(BaseVisitor, Stack):
    ...
    def visitMethodDecl(self, ast, c):
        ...
        #     print(f"param {param.accept(self,local)}")
    ...
    def visitBinaryOp(self, ast, c):
        ...
        if left not in primitive + ops or right not in primitive + ops:
            if left == "Id":
                # print(f"checking {ast.left}")
                res = self.lookupVariable(ast.left.accept(self, c), c)
                if res[0]:
                    if res[2] > 0:
                        if not c[res[2]]["const"]:
                            # print(f"in local but not immutable")
                            isStatic = False
                        # print(f"in local and immutable")
                    else:
                        if not c[0][res[3]]["const"]:
                            # print(f"in global but not immutable")
                            isStatic = False
                        # print(f"in global and immutable")
                else:
                    print(f"not found anywhere???")
            elif right == "Id":
                # print(f"checking {ast.right}")
                res = self.lookupVariable(ast.right.accept(self, c), c)
                if res[0]:
                    if res[2] > 0:
                        if not c[res[2]]["const"]:
                            # print(f"in local but not immutable")
    ...
    def visitFieldAccess(self, ast, c):
        raise Undeclared(Method(), "DEBUGGIN")
